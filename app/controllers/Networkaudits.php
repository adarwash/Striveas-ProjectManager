...
class Networkaudits extends Controller {
	...
	public function edit($id = null) {
		if (!hasPermission('clients.update')) {
			flash('client_error', 'You do not have permission to edit audits.', 'alert-danger');
			redirect('clients');
			return;
		}
		if (!$id) {
		...
		$audit['servers_virtual_decoded'] = !empty($audit['servers_virtual']) ? json_decode($audit['servers_virtual'], true) : [];
		$audit['endpoints_workstations_decoded'] = !empty($audit['endpoints_workstations']) ? json_decode($audit['endpoints_workstations'], true) : [];
		$audit['engineer_ids_array'] = !empty($audit['engineer_ids']) ? json_decode($audit['engineer_ids'], true) : [];
		$audit['top_connection_types_array'] = !empty($audit['top_connection_types']) ? explode(',', $audit['top_connection_types']) : [];
		$audit['cloud_platforms_array'] = !empty($audit['cloud_platforms']) ? explode(',', $audit['cloud_platforms']) : [];

		$data = [
			'title' => 'Edit Network Infrastructure Discovery',
			'audit' => $audit,
			'clients' => $clients,
			'users' => $users,
		];

		$this->view('network_audits/edit', $data);
	}

	public function update($id = null) {
		if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !$id) {
			redirect('networkaudits/edit/' . ($id ?? ''));
			return;
		}
		if (!hasPermission('clients.update')) {
			flash('client_error', 'You do not have permission to update audits.', 'alert-danger');
			redirect('networkaudits/show/' . $id);
			return;
		}
		...
		// Same processing logic as store()
		...
		// Preserve existing JSON if the section was not posted at all (prevents accidental clearing)
		$serversPhysicalJson = $audit['servers_physical'];
		if (isset($_POST['servers_physical']) && is_array($_POST['servers_physical'])) {
			...
			$serversPhysicalJson = $rows ? json_encode($rows) : $serversPhysicalJson;
		}
		$serversVirtualJson = $audit['servers_virtual'];
		if (isset($_POST['servers_virtual']) && is_array($_POST['servers_virtual'])) {
			...
			$serversVirtualJson = $rows ? json_encode($rows) : $serversVirtualJson;
		}
		$endpointsWorkstationsJson = $audit['endpoints_workstations'];
		if (isset($_POST['endpoints_workstations']) && is_array($_POST['endpoints_workstations'])) {
			...
			$endpointsWorkstationsJson = $rows ? json_encode($rows) : $endpointsWorkstationsJson;
		}

		$success = $this->auditModel->update((int)$id, [
			'client_id' => $clientId,
		...
		if (!$success) {
			flash('client_error', 'Failed to update discovery form.', 'alert-danger');
			redirect('networkaudits/edit/' . $id);
			return;
		}

		flash('client_success', 'Network Infrastructure Discovery Form updated successfully.', 'alert-success');
		redirect('networkaudits/show/' . $id);
	}
