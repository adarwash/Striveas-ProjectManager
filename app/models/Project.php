...
class Project {
    ...
    public function create($data) {
        $query = "INSERT INTO projects (title, description, start_date, end_date, status, user_id, created_at) 
                 VALUES (?, ?, ?, ?, ?, ?, GETDATE())";
    ...
    public function update($data) {
        $query = "UPDATE projects 
                 SET title = ?, description = ?, start_date = ?, end_date = ?, status = ?, updated_at = GETDATE()
                 WHERE id = ?";
    ...
    public function getProjectStats() {
        ...
        return !empty($result) ? (object)$result[0] : (object)['active_count' => 0, 'completed_count' => 0, 'on_hold_count' => 0];
    }
} 